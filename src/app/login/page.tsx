...
export default function Page() {
  ...
  // Chequea si hay sesión al cargar
  useEffect(() => {
  ...
  // Efecto para registrar en el backend CUANDO el usuario cambia de null a un objeto
  useEffect(() => {
    // Solo se ejecuta si hay un usuario y antes no lo había (para evitar múltiples llamadas)
    if (user) {
      registerInBackend();
    }
  }, [user]); // Dependencia: se ejecuta cuando 'user' cambia

  // Función para manejar el login (sin cambios significativos)
  ...
  // Función para registrar en backend (con delay para sync)
  const registerInBackend = async () => {
    if (!user) return;

    // Delay de 1 segundo para dar tiempo a Supabase a sincronizar después de redirect
    await new Promise((resolve) => setTimeout(resolve, 1000));

    // Fetch el perfil completo del usuario
    const { data: fullUserData, error: userError } =
      await supabase.auth.getUser();
    if (userError) {
      console.error("Error fetching full user:", userError.message);
    ...
    if (!fullUser) {
      console.error("No se pudo obtener el usuario completo");
      console.log("No se pudo obtener el usuario completo");
      return;
    }

    // Extrae el Google ID real de identities
    const googleIdentity = fullUser.identities?.find(
      (identity) => identity.provider === "google"
    );
    const googleId = googleIdentity?.id || ""; // Cambiado a .id en lugar de .provider_id

    // Extrae nombre
    const nombre =
      fullUser.user_metadata?.full_name ||
      fullUser.user_metadata?.name ||
      fullUser.email?.split("@")[0] || // Usamos optional chaining '?'
      "Usuario Anónimo"; // Fallback final

    // Obtén el access_token fresco
    const {
      data: { session },
    } = await supabase.auth.getSession();
    ...
    console.log("Identities:", fullUser.identities);
    console.log("Google ID:", googleId);
    console.log("Access Token (length):", accessToken.length); // No loguear el token completo por seguridad
    console.log("Nombre:", nombre);
    console.log("Email:", fullUser.email);

    if (!googleId || !accessToken) {
      console.error("Falta Google ID o access_token");
      console.log("Falta Google ID o access_token - verifica consola");
      return;
    }

    // --- Guardar información en cookies ---
    Cookies.set("user_name", nombre, { expires: 7 }); // Expira en 7 días
    if (fullUser.email) {
      Cookies.set("user_email", fullUser.email, { expires: 7 });
    }
    if (googleId) {
      Cookies.set("google_id", googleId, { expires: 7 });
    }
    ...
    if (response.ok) {
      console.log("Usuario registrado en backend exitosamente");
    } else {
      const errorData = await response.json();
      console.error("Error registrando en backend:", errorData.error);
      console.log(`Error en backend: ${errorData.error}`);
    }
  };
