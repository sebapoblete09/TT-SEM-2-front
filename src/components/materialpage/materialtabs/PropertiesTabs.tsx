  ...
  return (
    <div className="space-y-4 mt-6">
      ...
      <Card>
        ...
        <CardContent>
          <dl className="grid grid-cols-2 gap-4">
            {Object.entries(mecanicas).map(([key, value]) => (
              <div key={key}>
                <dt className="text-sm font-medium text-muted-foreground capitalize">
                  {key.replace("_", " ")}
                </dt>
                <dd className="text-lg font-semibold mt-1">{value || "N/A"}</dd>
              </div>
            ))}
          </dl>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <dl className="grid grid-cols-2 gap-4">
            {Object.entries(perceptivas).map(([key, value]) => (
              <div key={key}>
                <dt className="text-sm font-medium text-muted-foreground capitalize">
                  {key.replace("_", " ")}
                </dt>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {Object.entries(emocionales).map(([key, value]) => (
            <div key={key}>
              <dt className="text-sm font-medium text-muted-foreground capitalize">
                {key.replace("_", " ")}
              </dt>
